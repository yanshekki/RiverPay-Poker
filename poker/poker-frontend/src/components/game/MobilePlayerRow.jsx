  ...
  return (
    <motion.div
      ...
      className={`flex items-center gap-2 sm:gap-3 p-2 sm:p-3 rounded-2xl border transition-all ${
        isTurn ? 'border-rp-cyan neon-cyan bg-rp-cyan/5' :
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <p className={`text-xs sm:text-sm font-mono truncate ${isMe ? 'text-rp-cyan' : 'text-neutral-300'}`}>
          {isMe ? `👤 ${t('game.you')}` : `${player.address?.substring(0, 6)}...`}
        </p>
        <p className="text-xs text-rp-light font-bold">${player.chips?.toLocaleString()}</p>
      </div>
      <div className="text-right shrink-0">
        {player.bet > 0 && <span className="text-xs text-rp-cyan font-bold">Bet ${player.bet}</span>}
        {player.lastAction && (
          <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-bold uppercase ml-1 ${
            player.isFolded ? 'bg-white/10 text-neutral-500' : 'bg-rp-cyan/20 text-rp-cyan'
